...
fn is_save(report: &mut Vec<i32>) -> bool {
    if report[0] < report[report.len() - 1] {
        return is_save_increasing(report, false);
...
fn is_save_increasing(report: &Vec<i32>, failure_limit_reached: bool) -> bool {
    for report_index in 0..report.len() - 1 {
        ...
        if report_diff <= 0 || report_diff > 3 {
            ...
            let mut cleaned_report = report.to_vec();
            cleaned_report.remove(report_index);
            let mut cleaned_report2 = report.to_vec();
            cleaned_report2.remove(report_index + 1);
            return is_save_increasing(&cleaned_report, true) || is_save_increasing(&cleaned_report2, true)
        }
    }
    true
}
